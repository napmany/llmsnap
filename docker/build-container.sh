...
# Set llmsnap repository, automatically uses GITHUB_REPOSITORY variable
# to enable easy container builds on forked repos
LS_REPO=${GITHUB_REPOSITORY:-napmany/llmsnap}
...
for CONTAINER_TYPE in non-root root; do
  ...
  docker build -f llmsnap.Containerfile --build-arg BASE_TAG=${BASE_TAG} --build-arg LS_VER=${LS_VER} --build-arg UID=${USER_UID} \
